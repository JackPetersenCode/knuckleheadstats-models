"""V6 training: v5 features + platoon, arsenal, pitcher H/A, streak, h2h, standings."""
import os
import numpy as np
import pandas as pd
import psycopg2
...
def engineer(df):
    ...
    for prefix in ("h_p", "a_p"):
        df[f"{prefix}_era_10"] = per9(df[f"{prefix}_er_10"], df[f"{prefix}_ip_10"])
        df[f"{prefix}_k9_10"]  = per9(df[f"{prefix}_k_10"],  df[f"{prefix}_ip_10"])
        df[f"{prefix}_bb9_10"] = per9(df[f"{prefix}_bb_10"], df[f"{prefix}_ip_10"])
        df[f"{prefix}_hr9_10"] = per9(df[f"{prefix}_hr_10"], df[f"{prefix}_ip_10"])
        df[f"{prefix}_ipgs_10"] = df[f"{prefix}_ip_10"] / df[f"{prefix}_starts_10"]
        df[f"{prefix}_era_sd"] = per9(df[f"{prefix}_er_sd"], df[f"{prefix}_ip_sd"])
        df[f"{prefix}_k9_sd"]  = per9(df[f"{prefix}_k_sd"],  df[f"{prefix}_ip_sd"])
        df[f"{prefix}_bb9_sd"] = per9(df[f"{prefix}_bb_sd"], df[f"{prefix}_ip_sd"])
        df[f"{prefix}_hr9_sd"] = per9(df[f"{prefix}_hr_sd"], df[f"{prefix}_ip_sd"])

    for prefix in ("h_bp", "a_bp"):
        df[f"{prefix}_era_14"] = per9(df[f"{prefix}_er_14"], df[f"{prefix}_ip_14"])
        df[f"{prefix}_k9_14"]  = per9(df[f"{prefix}_k_14"],  df[f"{prefix}_ip_14"])
        df[f"{prefix}_bb9_14"] = per9(df[f"{prefix}_bb_14"], df[f"{prefix}_ip_14"])
    ...
    diffs = {
        ...
        "d_gb": ("a_gb", "h_gb"),  # +ve = away further from leader
    ...
    # v6: matchup using lineup vs hand
...
# Build feature list dynamically by enumerating relevant columns
def feature_list(df):
    base = [
...
def backtest_table(df, prob_col, thresholds=(0.00, 0.02, 0.04, 0.06, 0.08, 0.10)):
...
def fit_predict(Xtr, ytr, Xva, yva, Xtest_dict):
    ...
    out = {}
    lr = LogisticRegression(C=0.1, max_iter=5000).fit(Xtr_s, ytr)
    ...
    xgbm = xgb.XGBClassifier(
        ...
        subsample=0.85, colsample_bytree=0.6, reg_lambda=3.0, reg_alpha=0.5,
        min_child_weight=10, objective="binary:logistic", eval_metric="logloss",
        early_stopping_rounds=50, random_state=42, n_jobs=4, tree_method="hist")
    ...
    lgbm = lgb.LGBMClassifier(
        n_estimators=1500, num_leaves=31, max_depth=-1, learning_rate=0.02,
        subsample=0.85, colsample_bytree=0.6, reg_lambda=3.0, reg_alpha=0.5,
        min_child_samples=25, random_state=42, n_jobs=4, verbose=-1)
    lgbm.fit(Xtr_i, ytr, eval_set=[(Xva_i, yva)], callbacks=[lgb.early_stopping(50)])
...
def main():
    ...
    feats_mkt = feats + ["mkt_logit","open_logit","line_move"]
    print(f"Total games: {len(df)}  features: {len(feats)} (no-mkt), {len(feats_mkt)} (with mkt)")
    ...
    for label, feat_set in [("STATS-ONLY", feats), ("STATS+MARKET", feats_mkt)]:
        print("\n" + "="*72)
        ...
        out, xgbm = fit_predict(Xt, yt, Xv, yv, Xte_dict)
        ...
        if label == "STATS+MARKET":
            ...
            fi = sorted(zip(feat_set, xgbm.feature_importances_), key=lambda x: -x[1])
